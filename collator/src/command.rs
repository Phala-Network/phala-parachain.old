...
impl SubstrateCli for Cli {
	...
	fn support_url() -> String {
		"https://github.com/Phala-Network/phala-parachain/issues/new".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		load_spec(id, self.run.parachain_id.unwrap_or(1030).into())
	}
	...
	match &cli.subcommand {
		Some(Subcommand::BuildSpec(cmd)) => {
			...
			let chain_spec = load_spec(
				...
				cmd.parachain_id.unwrap_or(1030).into()
			)?;
		...
		Some(Subcommand::ExportGenesisState(params)) => {
			...
			let block: Block = generate_genesis_block(&load_spec(
				...
				params.parachain_id.unwrap_or(1030).into(),
			)?)?;
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				let id = ParaId::from(cli.run.parachain_id.or(para_id).unwrap_or(1030));
