...
fn purge_chain_works() {
	...
	// Check that both databases are deleted
	{
		...
		assert!(base_path.path().join("chains/phala_local_testnet/db").exists());
		assert!(base_path.path().join("polkadot/chains/westend_dev/db").exists());

		let status = Command::new(cargo_bin("phala-collator"))
			.args(&["purge-chain", "-d"])
			.arg(base_path.path())
			.arg("-y")
			.status()
		...
		assert!(base_path.path().join("chains/phala_local_testnet").exists());
		assert!(!base_path.path().join("chains/phala_local_testnet/db").exists());
